...
import { fetchSellerData, fetchSellerDatas } from '@/app/redux/features/seller/sellerSlice'
...
interface EnhancedTableProps {
    numSelected: number;
    onRequestSort: (event: React.MouseEvent<unknown>, property: keyof Seller) => void;
    order: Order;
    orderBy: string;
    rowCount: number;
}
...
function EnhancedTableHead(props: EnhancedTableProps) {
    ...
    const createSortHandler =
        (property: keyof Seller) => (event: React.MouseEvent<unknown>) => {
...
export default function SellerTable() {
    ...
    const [orderBy, setOrderBy] = React.useState<keyof Seller>('seller_code');
    ...
    const { sellerList, loading, error } = useSelector((state: RootState) => state.seller);
    React.useMemo(() => {
    ...
    const rows: Pagination<Seller> | null = sellerList;
    const handleRequestSort = (
        event: React.MouseEvent<unknown>,
        property: keyof Seller,
    ) => {
    ...
    const visibleRows = React.useMemo(
        ...
        [order, orderBy, page, rowsPerPage, sellerList],
    );
    ...
    //     await dispatch(fetchSellerDatas(API_URL));
    // };
    ...
    const handleClickOpen = async (id: string) => {
        setSelectId(id)
        setEditOpen(true)
        await dispatch(fetchSellerData(id));
    };
                                    ...
                                    >
                                        <TableCell padding="checkbox">
                                            <Stack direction="row" spacing={-1.5}>
                                                ...
                                                {(selectId == row.id) && <SellerEditComponent editOpen={editOpen} editIsError={editIsError} onClose={handleClose} />}
